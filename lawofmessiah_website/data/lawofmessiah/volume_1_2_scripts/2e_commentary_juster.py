...
def extract_commentary_from_html(filepath):
    with open(filepath, "r", encoding="ISO-8859-1") as file:
        ...
        # Extract commentary
        commentary = []
        commentary_start = soup.find("b", text="Commentary by Daniel C. Juster")
        if commentary_start:
            for tag in commentary_start.find_all_next("p"):
                if any(tag.find("b", text=txt) for txt in ["Classical Commentators", "NCLA", "Commentary by Rudolph", "Commentary of Rudolph", "Rudolph", "Commentary by Dr. Rudolph", "Rudolph"]) or tag.find("i", text="NOTE BY RUDOLPH"):
                    break
        ...
        return {
            ...
            "commentary_juster": commentary_html
        }
...
with open(os.path.join(output_dir, "output_2e_commentary.yaml"), "w") as yaml_file:
    yaml_file.write(yaml_data)

print("YAML data has been written to volume_1_2_output/output_2e_commentary.yaml")