...
def _allowed_target_audiences(request):
    kids_mode = str(request.COOKIES.get('jc_kids_mode', '')).lower() == 'true'
    return {'any', 'kids'} if kids_mode else {'any', 'adults'}


def _allowed_media_languages(request):
    language = str(getattr(request, 'LANGUAGE_CODE', 'en') or 'en').split('-')[0].lower()
    # Drawings and legacy resources are often tagged in English only.
    return {'any', 'unknown', 'en', language}


def _filter_grouped_media_by_audience(grouped_media, allowed_audiences, allowed_languages):
    filtered = {}
    for media_type, media_items in (grouped_media or {}).items():
        filtered[media_type] = [
            item for item in media_items
            if item.get('is_public')
            and item.get('target_audience', 'any') in allowed_audiences
            and item.get('language', 'any') in allowed_languages
        ]
    return filtered
...
class LawOfMessiahDetailView(View):
    def get(self, request, law_id: str):
        ...
        law = get_object_or_404(
            LawOfMessiah.objects.prefetch_related('bible_reference_rows', 'media'),
            pk=law_id,
        )
        ...
        # Build enriched Maimonides list (deduplicated, with commandment_type).
        ...
        try:
            ...
            for ref in refs:
                ...
                try:
                    ...
                    text = _('Could not load Bible verse for this reference.')
                    source = 'error'
                verses[ref_key] = text
